...
public class MiningRespond extends TaskRespond {

	public MiningRespond(String respond) {
		super(respond);
	}
	...
	public void execute(PrintWriter out, BufferedReader in) throws IOException {
		...
		List<Integer> rocks = getRocks(parsedRocks);
		...
		SkillTask newTask = new MiningTask(actionArea, bankArea, rocks, axe);
	...
	private List<Integer> getRocks(String parsedRocks) {
		List<Integer> rocks = new ArrayList<Integer>();
		String[] splittedRocks = parsedRocks.split(",");
		for(String rock : splittedRocks) {
			if(rock != null) {
			rocks.add(Integer.parseInt(rock));
			}
		}
		return rocks;
	}
